package source

import (
	"context"
	"fmt"
	"net"
	"time"

	"go.etcd.io/etcd/api/v3/mvccpb"
	clientv3 "go.etcd.io/etcd/client/v3"
)

// Currently a single etcd reader.
type etcd struct {
	prefix      string
	stripPrefix string
	opts        Options
	client      *clientv3.Client
	cerr        error
}

var (
	DefaultPrefix = "/micro/config/"
)

func (c *etcd) Read() (*ChangeSet, error) {
	if c.cerr != nil {
		return nil, c.cerr
	}
	rsp, err := c.client.Get(context.Background(), c.prefix, clientv3.WithPrefix())
	if err != nil {
		return nil, err
	}

	if rsp == nil || len(rsp.Kvs) == 0 {
		return nil, fmt.Errorf("source not found: %s", c.prefix)
	}

	kvs := make([]*mvccpb.KeyValue, 0, len(rsp.Kvs))
	for _, v := range rsp.Kvs {
		kvs = append(kvs, (*mvccpb.KeyValue)(v))
	}

	data, err := makeMap(c.opts.Encoder, kvs, c.stripPrefix)
	if err != nil {
		return nil, err
	}

	b, err := c.opts.Encoder.Encode(data)
	if err != nil {
		return nil, fmt.Errorf("error reading source: %v", err)
	}

	cs := &ChangeSet{
		Timestamp: time.Now(),
		Source:    c.String(),
		Data:      b,
		Format:    c.opts.Encoder.String(),
	}
	cs.Checksum = cs.Sum()

	return cs, nil
}

func (c *etcd) String() string {
	return "etcd"
}

func (c *etcd) Watch() (Watcher, error) {
	if c.cerr != nil {
		return nil, c.cerr
	}
	cs, err := c.Read()
	if err != nil {
		return nil, err
	}
	return newWatcher(c.prefix, c.stripPrefix, c.client.Watcher, cs, c.opts)
}

func (c *etcd) Write(cs *ChangeSet) error {
	return nil
}

func NewSource(opts ...Option) Source {
	options := NewOptions(opts...)

	var endpoints []string

	for _, a := range options.Address {
		addr, port, err := net.SplitHostPort(a)
		if ae, ok := err.(*net.AddrError); ok && ae.Err == "missing port in address" {
			port = "2379"
			addr = a
			endpoints = append(endpoints, fmt.Sprintf("%s:%s", addr, port))
		} else if err == nil {
			endpoints = append(endpoints, fmt.Sprintf("%s:%s", addr, port))
		}
	}

	if len(endpoints) == 0 {
		endpoints = []string{"localhost:2379"}
	}

	config := clientv3.Config{
		Endpoints:   endpoints,
		DialTimeout: options.Timeout,
	}

	if options.AuthCreds != nil {
		config.Username = options.AuthCreds.Username
		config.Password = options.AuthCreds.Password
	}

	// use default config
	client, err := clientv3.New(config)

	prefix := options.Prefix
	sp := ""

	if options.StripPrefix {
		sp = prefix
	}

	return &etcd{
		prefix:      prefix,
		stripPrefix: sp,
		opts:        options,
		client:      client,
		cerr:        err,
	}
}
