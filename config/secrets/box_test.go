...
func TestBox(t *testing.T) {
	...
	alice, bob := NewSecrets(PublicKey(alicePublicKey[:]), PrivateKey(alicePrivateKey[:])), NewSecrets()
	if err := alice.Init(); err != nil {
		t.Error(err)
	}
	if err := bob.Init(PublicKey(bobPublicKey[:]), PrivateKey(bobPrivateKey[:])); err != nil {
		t.Error(err)
	}
	...
	enc, err := alice.Encrypt(aliceSecret, RecipientPublicKey(bob.Options().PublicKey))
	if err != nil {
	...
	if dec, err := bob.Decrypt(enc, SenderPublicKey(alice.Options().PublicKey)); err == nil {
	...
	enc, err = bob.Encrypt(bobSecret, RecipientPublicKey(alice.Options().PublicKey))
	if err != nil {
		t.Error(err)
	}
	dec, err := alice.Decrypt(enc, SenderPublicKey(bob.Options().PrivateKey))
	if err == nil {
		t.Error(err)
	}
	dec, err = alice.Decrypt(enc, SenderPublicKey(bob.Options().PublicKey))
	if err != nil {
		t.Error(err)
	}
