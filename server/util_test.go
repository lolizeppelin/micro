...
func TestUtils(t *testing.T) {
	...
	fmt.Printf("now 12 se: %0*s\n", 12, utils.ToBase62(uint(time.Now().Unix())))
	fmt.Printf("now 12 ms: %0*s\n", 12, utils.ToBase62(uint(time.Now().UnixMicro())))
	fmt.Printf("now 15 ms: %0*s\n", 15, utils.ToBase62(uint(time.Now().UnixMicro())))
	fmt.Printf("now 16 ms: %0*s\n", 16, utils.ToBase62(uint(time.Now().UnixMicro())))
}
