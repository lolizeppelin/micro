...
type Cache interface {
	GetService(service string) ([]*micro.Service, error)
	Stop()
}
...
type cache struct {
	...
	// registry cache
	sync.RWMutex
	cache   map[string][]*micro.Service
...
func (c *cache) isValid(services []*micro.Service, ttl time.Time) bool {
	// no services exist
	if len(services) == 0 {
		return false
	}
...
func (c *cache) get(service string) ([]*micro.Service, error) {
	// read lock
	c.RLock()
	...
	get := func(service string, cached []*micro.Service) ([]*micro.Service, error) {
		// ask the registry
		val, err, _ := c.sg.Do(service, func() (interface{}, error) {
			return c.registry.GetService(service)
		})
		services, _ = val.([]*micro.Service)
		if err != nil {
			// check the cache
			if len(cached) > 0 {
...
func (c *cache) set(service string, services []*micro.Service) {
	c.cache[service] = services
	c.ttls[service] = time.Now().Add(c.TTL)
}

func (c *cache) update(res *micro.Result) {
...
func (c *cache) run(service string) {
	...
	for {
		...
		// create new watcher
		w, err := c.registry.Watch(service)
		if err != nil {
			if c.quit() {
				return
			}
...
func (c *cache) watch(w micro.Watcher) error {
...
func (c *cache) GetService(service string) ([]*micro.Service, error) {
	// get the service
...
func New(r micro.Registry, ttl time.Duration) Cache {
	...
	return &cache{
		...
		cache:          make(map[string][]*micro.Service),
